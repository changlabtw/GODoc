function [] = seq_eval(matlab_path, cat, ont_db_path, oa_file, pred_file, benchmark_file, output_folder, evmode)
  addpath(matlab_path)
  if nargin < 8
    evmode = '1'
  else
  ...
  pr = pfp_seqmetric(benchmark, pred, oa, 'pr', 'evmode', evmode);
  fmax
  ...
  if evmode == '1'
    ...
    writetable(array2table(pr,'VariableNames',{'precision','recall'}), strcat(output_folder,'/partial_prcurve.csv'));
  end
end
