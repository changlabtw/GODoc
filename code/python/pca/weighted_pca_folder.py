...
def process_options():
    ...
    parser.add_argument('-w','--weight', help='weight for d = 3', type=float, default='0.0')
...
def load_vector(ID):
    ...
    if args.weight != 0:
        d = len(cur_vector)/400
        for ind, freq in enumerate(cur_vector):
            if ind%d <= 3:
                cur_vector[ind] = freq*args.weight
