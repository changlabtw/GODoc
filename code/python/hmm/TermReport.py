...
#python ../TermReport.py -i hmm_scan_result.txt -l new_propagated_MFO.txt -m mfo_model_list.txt -t mfo_all_typex.txt -db ../../propagate/go-basic.obo
#python ../TermReport.py -i hmm_scan_result.txt -l new_propagated_CCO.txt -m cco_model_list.txt -t cco_all_typex.txt -db ../../propagate/go-basic.obo
#python ../TermReport.py -i hmm_scan_result.txt -l new_propagated_BPO.txt -m bpo_model_list.txt -t bpo_all_typex.txt -db ../../propagate/go-basic.obo
...
def main():
    ...
    for i in range(0,len(report_df.index)):
        ...
        report_df.loc[i,['TP']] = TP_amount
        FP_amount = len(curFP_df.index)
        report_df.loc[i,['FP']] = FP_amount
        if TP_amount != 0 or FP_amount !=0:
            ...
            if TP_amount != 0:
                ...
                report_df.loc[i,['TP_std','TP_min','TP_25%','TP_50%','TP_75%','TP_max']] = [TP_distri['std'],TP_distri['min'],TP_distri['25%'],TP_distri['50%'],TP_distri['75%'],TP_distri['max']]
            if FP_amount != 0:
                FP_distri = distri_df[distri_df['label']=='FP']['b_E-value'].describe()
                report_df.loc[i,['FP_std','FP_min','FP_25%','FP_50%','FP_75%','FP_max']] = [FP_distri['std'],FP_distri['min'],FP_distri['25%'],FP_distri['50%'],FP_distri['75%'],FP_distri['max']]

        #FN
        FN_amount = len(label_df[label_df['GO'] == cur_go].index)
        report_df.loc[i,['FN']] = FN_amount
        ...
        report_df.loc[i,['precision','recall','fmeasure']] = [cur_pr, cur_rc, cur_f]

        #model_count
        report_df.loc[i,['model_amount']] = model_list.count(cur_go)

        #depth at ontology
        report_df.loc[i,['depth']] = godb.query_term(cur_go).depth
