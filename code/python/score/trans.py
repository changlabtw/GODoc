...
def parse(ind):
    ...
    for go in gos:
        str_list.append(pred_res_df.loc[ind, 'ID'] + '\t' + go + '\t' + '1.00' + '\n')
...
def main():
    ...
    print "Unpred protein amount: %i" % len(pred_res_df[pred_res_df['pred_class'].isnull()].index)
    pred_res_df = pred_res_df[-pred_res_df['pred_class'].isnull()].reset_index(drop=True) #drop unpred protein
