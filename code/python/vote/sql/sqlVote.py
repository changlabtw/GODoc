...
def knn(test_vec, train_vecs, k = 3):
    ...
    for i in range(0,len(train_vecs.index)):
        ...
        if len(knn_df.index) < k:
            ...
            knn_df = knn_df.sort_values(by = ["dist"])
...
def knn_vote(test_vec, train_vecs, k):
    ...
    train_label_conn = sqlite3.connect('db/train.db')
    test_label_conn = sqlite3.connect('db/test.db')
    for j in range(0,len(dist_df.index)):
        current_gos = queryID(train_label_conn, dist_df.loc[j,"ID"])
...
def queryID(conn, ID):
    cursor = conn.execute("SELECT GO from label where ID=?",(ID,))
