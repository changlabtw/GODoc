    ...
    do
      ...
      do
        if [ "$whiten" == "-w" ]; then
          ...
          model_path=thesis/data/cafa3_train/nfold/$t/fold$fold/exp/1/w
        else
          cur_path=thesis/data/cafa3_train/nfold/$t/fold$fold/exp/${explain}/nw
          model_path=thesis/data/cafa3_train/nfold/$t/fold$fold/exp/1/nw
        fi
        # Non-redundant
        python python/pca/pca_exp.py -i thesis/data/cafa3_train/tfpssm/ -n $explain \
