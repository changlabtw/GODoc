...
if [ "$#" -ne 4 ]; then
    ...
    echo "Usage: ./pca_dim.sh [nfold folder root] [data folder root] [output] [type]"
    exit
fi
...
touch ${output}
echo type,fold,ratio,whiten,svd,dim >> ${output}
  ...
  do
    ...
    do
      ...
      do
        for svd in r nr
        do
          res=${nfold_root}/${type}/fold${fold}/${data_root}/${ratio}/${whiten}/${svd}/test_tfpssm_pca.csv
          if [ ! -f ${res} ]
          then
            echo "${res} not found!"
          else
            echo -n ${type},${fold},${ratio},${whiten},${svd}, >> ${output}
            head -n 1 -q ${res} | tr -cd , | wc -c >> ${output}
          fi
        done
      done
    done
